...
<?PHP
	...
	$test_os = $_GET['selected_testos'];

	include 'db-open.php';

	if ( isset($DEBUG) ) { echo "<FONT SIZE=\"-1\">Variables: OrderBy='<B>$OrderBy</B>', Resource='<B>$Resource</B>', test_os='<B>$test_os</B>'</FONT><BR>\n"; }
	...
	$query = "UPDATE $table_name SET test_os='$test_os' WHERE id=$Resource";
	...
	if ( $Resource && $test_os ) {
		...
		if ($result) {
			...
			echo "<H2 ALIGN=\"center\">Current Test OS Change: <FONT COLOR=\"green\">Success</FONT></H2>\n";
		} else {
			if ( isset($DEBUG) ) { echo "<FONT SIZE=\"-1\">Query: <B>ERROR</B>, Resource Update -> <B>FAILED</B></FONT><BR>\n"; }
			echo "<H2 ALIGN=\"center\">Current Test OS Change: <FONT COLOR=\"red\">FAILED</FONT></H2>\n";
			echo "<P ALIGN=\"center\"><B>ERROR:</B> " . mysql_error() . "</P>\n";
		}
	} else {
		if ( isset($DEBUG) ) { echo "<FONT SIZE=\"-1\"><B>ERROR</B>: Undefined Variable(s); Resource or test_os</FONT><BR>\n"; }
		echo "<H2 ALIGN=\"center\">Current Test OS Change: <FONT COLOR=\"red\">FAILED</FONT></H2>\n";
