...
<?PHP
	...
	if ( $owner ) {
		...
		$query = "UPDATE $table_name SET status='Extended',due_date=DATE_ADD(CURDATE(), INTERVAL 7 DAY) WHERE id=$Resource";
		if ( isset($DEBUG) ) { echo "<FONT SIZE=\"-1\">Query: Submitted -> <B>$query</B></FONT><BR>\n"; }
		if ( $Resource ) {
			$result=mysql_query($query);
			if ($result) {
				if ( isset($DEBUG) ) { echo "<FONT SIZE=\"-1\">Query: Resource Update -> <B>Success</B></FONT><BR>\n"; }
				echo "<H2 ALIGN=\"center\">Resource Extended: <FONT COLOR=\"green\">Success</FONT></H2>\n";
			} else {
				if ( isset($DEBUG) ) { echo "<FONT SIZE=\"-1\">Query: <B>ERROR</B>, Resource Update -> <B>FAILED</B></FONT><BR>\n"; }
				echo "<H2 ALIGN=\"center\">Resource Extended: <FONT COLOR=\"red\">FAILED</FONT></H2>\n";
		...
		echo "<H2 ALIGN=\"center\">Resource Extended: <FONT COLOR=\"red\">NOTICE</FONT></H2>\n";
		echo "<P ALIGN=\"center\"><B>NOTICE:</B> The resource is unassigned, not extended.</P>\n";
	}
